...
include { MANTA_NOCONTROL as MANTA ; FILTER_MANTA_NOCONTROL as FILTER_MANTA; MERGE_MANTA_RESULTS} from '../modules/local/manta'
include { FREEC_NOCONTROL as FREEC; MERGE_FREEC_RESULTS } from '../modules/local/freec'
include { PLOT_CHR_MANTA_FREEC; PLOT_CIRCOS_MANTA_FREEC } from '../modules/local/plot'
...
workflow WGS_NOCONTROL{
  ...
  FREEC(input,ref_fa,ref_fai,freec_template_nocontrol,freec_gc,freec_len)

  manta_pon = Channel.fromPath(params.manta_pon).collect()
  FILTER_MANTA(MANTA.out,manta_pon)

  plot_input = FREEC.out.combine(FILTER_MANTA.out.SVfiltered,by:0)
