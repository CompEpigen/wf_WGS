...
#parser.add_argument('-oUncertain', type = str, help='Output VCF of uncertain SV calls')
...
parser.add_argument('--pon', type = str, help='bedpe file containing regions to filter out.')
...
parser.add_argument('--filterSmallInsertions', type = str,default="true", help='If set to 0, will not filter out SVs resulting in small insertions.')
parser.add_argument('--puretumor', type = str,default="false", help='If set to 1, will assume that the tumor is pure and will not filter out when no reads support the reference allele..')
...
#    header.samples.names = [args.sample]
#    header.samples.name_to_idx = {args.sample: 0}
writer = vcfpy.Writer.from_path(args.o,header)
#writer_uncertain = vcfpy.Writer.from_path(args.oUncertain,header)
n_filter_PoN = 0
n_filter_length=0
n_filter_support=0
n_filter_polyA=0
for record in reader:
    ...
    if (PR < args.minPR and record.calls[args.tumorindex].data["ASRP"] <args.minPR) or SR<args.minSR:
        ...
        reliable = False
        continue
    if args.puretumor=="false" and record.calls[args.tumorindex].data["REF"]+record.calls[args.tumorindex].data["REFPAIR"]<2:
    ...
    # Only apply this filter if we do not have a control sample.
    if args.tumorindex==0 and reliable and (low_mapq_in_region(samfile,chr,pos-100,pos+100,30) or low_mapq_in_region(samfile,chr2,pos2-100,pos2+100,30)):
    ...
    #    print("Breakpoint close to gene")
        #writer.write_record(record)
        #continue
    if reliable and args.tumorindex==0:
        ...
        if filter_out_record and (args.filterSmallInsertions == "true"): 
    ...
    #else:
    #    print("Uncertain")
    #    write_record(writer_uncertain,record)
